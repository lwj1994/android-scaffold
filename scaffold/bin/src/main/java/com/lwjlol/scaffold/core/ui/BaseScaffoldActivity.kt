 ...
 */
abstract class BaseScaffoldActivity(
    private val layoutRes: Int = R.layout.scaffold_activity_empty
) : BaseMvRxActivity(), BaseView, OnKeyBoardVisibleListener {
    // contentView
    // 协程
    protected val lifeScope = CoroutineScope(SupervisorJob() + DISPATCHER_MAIN)
    protected val navController: NavController by lazy(LazyThreadSafetyMode.NONE) {
        Navigation.findNavController(activityRootView).apply {
            navigatorProvider.addNavigator(keepNavigator)
        }
    }
    private val keepNavigator: KeepStateNavigator by lazy(LazyThreadSafetyMode.NONE) {
        val navHostFragment =
            supportFragmentManager.findFragmentById(R.id.scaffold_nav_host_fragment)
                ?: throw IllegalArgumentException("Activity:${javaClass.name} must set fragment who id is scaffold_nav_host_fragment")
        KeepStateNavigator(
            viewContext,
            navHostFragment.childFragmentManager,
            R.id.scaffold_nav_host_fragment
        )
    }
    ...
    protected open val activityRootView: View by lazy(NONE) {
        findViewById<View>(R.id.scaffold_activity_base_rootView)
            ?: throw IllegalStateException("activity must set rootView who the id is scaffold_activity_base_rootView")
    }
    ...
    protected open fun initView() {
        setContentView(View.inflate(viewContext, layoutRes, null).apply {
            id = R.id.scaffold_activity_base_rootView
        })
    }
...
abstract class BaseFragment(private val layoutRes: Int) : BaseMvRxFragment(), BaseView {
...
abstract class BaseListFragment : BaseFragment(R.layout.scaffold_fragment_base_refreshlist) {
